...
public class ValueMappingSourceForm2 extends VerticalLayout {
    ...
    private Collection<MappingDefinition> filter(Set<MappingDefinition> model, String filterText) {
        ...
        return model.stream().filter(mappingDefinition->
                mappingDefinition.getDeviceType().getManufacturerName().contains(filterText) ||
